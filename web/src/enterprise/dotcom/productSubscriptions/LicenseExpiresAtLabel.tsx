...
export const LicenseExpiresAtLabel: React.FunctionComponent<{
    ...
    <span className={className}>
        {productSubscription.activeLicense && productSubscription.activeLicense.info ? (
            <>
                <Timestamp date={productSubscription.activeLicense.info.expiresAt} className={isBefore(new Date(productSubscription.activeLicense.info.expiresAt), Date.now()) ? 'text-danger' : ''}/>
            </>
        ):(
